			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="col-md-6">
							<form role="form" action="save-user.php?mode=edit" method="post">
								<?php
//-----------------------------------------GET DATA BARANG---------------------------------------------------------

								$id=$_GET['id'];
								$sqlusr="select * from user where username='$id'";
								$qrusr=mysqli_query($con,$sqlusr);
								$dtusr=mysqli_fetch_array($qrusr);							
								echo '
								<input type="hidden" name="id" value="'.$id.'">
								<div class="form-group">
									<label>Username</label>
									<input class="form-control" name="username" value="'.$dtusr['username'].'" readonly>
								</div>
								
								<div class="form-group">
									<label>Password Baru</label>&nbsp;(biarkan kosong jika tidak ingin mengganti)
									<input class="form-control" name="password" value="">
								</div>
															
								<div class="form-group">
									<label>Email</label>
									<input class="form-control" name="email" value="'.$dtusr['email'].'">
								</div>
								
								<div class="form-group">
									<label>Level (Hak Akses)</label>
									<select class="form-control" name="level">
									<option value="'.$dtusr['level'].'" selected>'.$dtusr['level'].'</option>

	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		...
		<div class="row">
			...
			for ($i=2; $i<=$baris; $i++)
			{
				...
				// membaca data nama barang (kolom ke-1)
 				$nama = $data->val($i, 1);
  			
				// membaca data satuan(kolom ke-2)
 				$satuan= $data->val($i, 2);
  				
				// membaca data nama harga beli (kolom ke-3)
 				$hbeli = $data->val($i, 3);
  				
				// membaca data harga jual (kolom ke-4)
  				$hjual = $data->val($i, 4);

  				// setelah data dibaca, sisipkan ke dalam tabel mhs
  				$query_check = "select nama_barang from barang where nama_barang like '%$nama%'";
  				$hasil_check = mysqli_query($con,$query_check);
  				if (mysqli_num_rows($hasil_check) > 0) {
  					$gagal_import++;
				}else{
			...
			echo "Terdapat data barang yang sama sejumlah : ".$gagal_import."</p>";
